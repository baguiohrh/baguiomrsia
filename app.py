...
def load_data_as_of():
  try:
    ...
    extract_date = (
        str(df_as_of.iloc[1, 0]).strip()
        if len(df_as_of) > 1 and len(df_as_of.columns) > 0
        else ""
    )
    extract_time = (
        str(df_as_of.iloc[1, 1]).strip()
        if len(df_as_of) > 1 and len(df_as_of.columns) > 1
        else ""
    )
...
if timestamp_str and timestamp_str != "nan nan":
  st.info(
      f"📌 **Data as of:** {timestamp_str} | *All data is subject to change"
...
if not df_raw.empty:
  ...
  bakuna_centers = sorted(
      df_for_bakuna[col_bakuna].dropna().astype(str).unique().tolist()
  )
  selected_bakuna = st.sidebar.multiselect(
      "Bakuna Center Name", bakuna_centers
  )

  # Filter 3: Barangay Name [Column H] (DEPENDENT ON CITY & BAKUNA CENTER SELECTION)
  df_for_barangay = df_for_bakuna.copy()
  if selected_bakuna:
    df_for_barangay = df_for_barangay[
        df_for_barangay[col_bakuna].astype(str).isin(selected_bakuna)
    ]

  barangays = sorted(
      df_for_barangay[col_barangay].dropna().astype(str).unique().tolist()
  )
  ...
  valid_dates = (
      df_raw[col_date].dropna() if col_date in df_raw.columns else pd.Series()
  )
  ...
  if selected_city:
    filtered_df = filtered_df[
        filtered_df[col_city].astype(str).isin(selected_city)
    ]

  if selected_bakuna:
    filtered_df = filtered_df[
        filtered_df[col_bakuna].astype(str).isin(selected_bakuna)
    ]

  if selected_barangay:
    filtered_df = filtered_df[
        filtered_df[col_barangay].astype(str).isin(selected_barangay)
    ]

  # Safe Date Filter Application
  if (
      len(selected_date_range) == 2
      and col_date in filtered_df.columns
      and not filtered_df.empty
  ):
    start_date, end_date = selected_date_range
    if not pd.api.types.is_datetime64_any_dtype(filtered_df[col_date]):
      filtered_df[col_date] = pd.to_datetime(
          filtered_df[col_date], errors="coerce"
      )
  ...
  response_series = (
      filtered_df[col_response].astype(str)
  ...
  mr_dose_cols = [
      ...
      if "MR" in c
      and "Zero" not in c
      and any(age in c for age in ["6-12", "13-23", "24-59"])
  ]
  ...
  col_zero_doses = (
      df_raw.columns[19]
  )  # "Grand total doses administered (Zero dose)"

  # Base totals
  total_doses = (
      clean_numeric_sum(filtered_df[col_total_doses])
      if not filtered_df.empty
      else 0
  )
  total_zero_doses = (
      clean_numeric_sum(filtered_df[col_zero_doses])
      if not filtered_df.empty
      else 0
  )
  ...
  vit_a_df = (
      filtered_df[
          response_series.str.contains("Vitamin A", case=False, na=False)
      ]
      if not filtered_df.empty
      else pd.DataFrame()
  )
  mr_df = (
      filtered_df[
          response_series.str.contains("Measles|MR", case=False, na=False)
      ]
      if not filtered_df.empty
      else pd.DataFrame()
  )
  ...
  if vit_a_target_cols and not vit_a_df.empty:
    ...
    if not vit_a_df_clean.empty:
      vit_a_df_clean["Daily_Vit_A_Total"] = vit_a_df_clean[
          vit_a_target_cols
      ].apply(
      ...
      daily_vit_a = (
          vit_a_df_clean.groupby(vit_a_df_clean[col_date].dt.date)[
              "Daily_Vit_A_Total"
          ]
          .sum()
          .reset_index()
  ...
  if all_mr_cols and not mr_df.empty:
    ...
    if not mr_df_clean.empty:
      ...
      daily_mr = (
          mr_df_clean.groupby(mr_df_clean[col_date].dt.date)["Daily_MR_Total"]
          .sum()
          .reset_index()
      )
  ...
  if not daily_vit_a.empty and not daily_mr.empty:
    daily_trend_df = pd.merge(
        daily_vit_a, daily_mr, on=col_date, how="outer"
    ).fillna(0)
  ...
  with col_summary1:
    if vit_a_target_cols:
      ...
      for col in vit_a_target_cols:
        ...
        vit_a_totals.append({
            "Metric": col.replace("Vitamin A ", "").replace(" Total", ""),
            "Total": int(val),
        })
  ...
  with col_summary2:
    if mr_dose_cols:
      ...
      for col in mr_dose_cols:
        ...
        mr_totals.append(
            {"Age": col.replace("MR ", "").replace(" Total", ""), "Total": int(val)}
        )
  ...
  with col_summary3:
    if mr_zero_cols:
      ...
      for col in mr_zero_cols:
        val = clean_numeric_sum(mr_df[col]) if not mr_df.empty else 0
        mr_zero_totals.append({
            "Age": col.replace("MR Zero ", "").replace(" Total", ""),
            "Total": int(val),
        })
  ...
  def create_gauge_chart(
      title, accomplishment, target, height=300, font_size=14, is_large=False
  ):
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            number={
                "valueformat": ",d",
                "font": {"size": 36 if is_large else 28},
            },
            domain={"x": [0.05, 0.95], "y": [0.05, 0.70]},
            title={
                "text": (
                    f"<b>{title}</b><br><span"
                    f" style='font-size:{0.95 if is_large else 0.85}em;color:#475569'>{pct}%"
                    f" of Target ({target:,})</span>"
                ),
                "font": {"size": font_size},
                "align": "center",
            },
    ...
    fig.update_layout(
        height=height, margin=dict(l=25, r=25, t=50 if not is_large else 60, b=30)
    )
    return fig
  ...
  if not df_target_raw.empty:
    ...
    t_city_col = next(
        (
            c
            for c in df_target_raw.columns
            if any(k in c.lower() for k in ["city", "muni"])
        ),
        None,
    )
    t_bakuna_col = next(
        (
            ...
            for c in df_target_raw.columns
            if any(
                k in c.lower() for k in ["bakuna", "dhc", "center", "facility"]
            )
        ),
        None,
    )
    ...
    if selected_barangay and t_barangay_col:
      selected_bgy_clean = [
          str(b).strip().upper() for b in selected_barangay
      ]
      gauge_target_df = gauge_target_df[
          gauge_target_df[t_barangay_col]
          .astype(str)
          .str.strip()
          .str.upper()
          .isin(selected_bgy_clean)
      ]
    else:
      if selected_city and t_city_col:
        selected_city_clean = [str(c).strip().upper() for c in selected_city]
        gauge_target_df = gauge_target_df[
            gauge_target_df[t_city_col]
            .astype(str)
            .str.strip()
            .str.upper()
            .isin(selected_city_clean)
        ]
      if selected_bakuna and t_bakuna_col:
        selected_bakuna_clean = [
            str(b).strip().upper() for b in selected_bakuna
        ]
        gauge_target_df = gauge_target_df[
            gauge_target_df[t_bakuna_col]
            .astype(str)
            .str.strip()
            .str.upper()
            .isin(selected_bakuna_clean)
        ]

    # Targets per Age Group (using updated column indexes)
    t_val_6_12 = (
        clean_numeric_sum(gauge_target_df[col_t_6_12]) if col_t_6_12 else 0
    )
    t_val_13_23 = (
        clean_numeric_sum(gauge_target_df[col_t_13_23]) if col_t_13_23 else 0
    )
    t_val_24_59 = (
        clean_numeric_sum(gauge_target_df[col_t_24_59]) if col_t_24_59 else 0
    )
    ...
    acc_val_6_12 = (
        clean_numeric_sum(mr_df[col_mr_6_12])
        if (not mr_df.empty and col_mr_6_12)
        else 0
    )
    acc_val_13_23 = (
        clean_numeric_sum(mr_df[col_mr_13_23])
        if (not mr_df.empty and col_mr_13_23)
        else 0
    )
    acc_val_24_59 = (
        clean_numeric_sum(mr_df[col_mr_24_59])
        if (not mr_df.empty and col_mr_24_59)
        else 0
    )
    ...
    with g_row1_col1:
      st.plotly_chart(
          create_gauge_chart(
              "6 - 12 mos Total<br><span"
              " style='font-size:0.8em;color:#64748b'>(Acc: Col K | Target: Col"
              " I)</span>",
              int(acc_val_6_12),
              int(t_val_6_12),
          ),
    ...
    with g_row1_col2:
      st.plotly_chart(
          create_gauge_chart(
              "13 - 23 mos Total<br><span"
              " style='font-size:0.8em;color:#64748b'>(Acc: Col L | Target: Col"
              " L)</span>",
              int(acc_val_13_23),
              int(t_val_13_23),
          ),
    ...
    with g_row1_col3:
      st.plotly_chart(
          create_gauge_chart(
              "24 - 59 mos Total<br><span"
              " style='font-size:0.8em;color:#64748b'>(Acc: Col M | Target: Col"
              " O)</span>",
              int(acc_val_24_59),
              int(t_val_24_59),
          ),
    ...
    st.plotly_chart(
        create_gauge_chart(
            "Overall Target: 6 - 59 mos Total<br><span"
            " style='font-size:0.75em;color:#64748b'>(Target: Col F)</span>",
            int(acc_val_6_59),
            int(t_val_6_59),
            height=420,
            font_size=18,
  ...
  st.info(
      "ℹ️ **Note:** Target is for the entire city of Baguio; no disaggregation"
      " per DHC or barangay."
  )
  ...
  vit_a_acc_6_11 = (
      clean_numeric_sum(filtered_df[col_vit_6_11])
      if (not filtered_df.empty and col_vit_6_11)
      else 0
  )
  vit_a_acc_12_59 = (
      clean_numeric_sum(filtered_df[col_vit_12_59])
      if (not filtered_df.empty and col_vit_12_59)
      else 0
  )
  ...
  if not df_target_vit_a.empty and len(df_target_vit_a) >= 1:
    target_vit_6_11 = (
        clean_numeric_sum(pd.Series([df_target_vit_a.iloc[0, 0]]))
        if len(df_target_vit_a.columns) > 0
        else 0
    )
    target_vit_12_59 = (
        clean_numeric_sum(pd.Series([df_target_vit_a.iloc[0, 1]]))
        if len(df_target_vit_a.columns) > 1
        else 0
    )
    target_vit_total = (
        clean_numeric_sum(pd.Series([df_target_vit_a.iloc[0, 2]]))
        if len(df_target_vit_a.columns) > 2
        else 0
    )
  else:
    target_vit_6_11 = 0
  ...
  with vg_col1:
    st.plotly_chart(
        create_gauge_chart(
            "Vit A 6 - 11 mos<br><span style='font-size:0.8em;color:#64748b'>(Acc:"
            " Col Q | Target: targetVitA A2)</span>",
            int(vit_a_acc_6_11),
            int(target_vit_6_11),
        ),
        use_container_width=True,
  ...
  with vg_col2:
    st.plotly_chart(
        create_gauge_chart(
            "Vit A 12 - 59 mos<br><span"
            " style='font-size:0.8em;color:#64748b'>(Acc: Col R | Target:"
            " targetVitA B2)</span>",
            int(vit_a_acc_12_59),
  ...
  st.plotly_chart(
      create_gauge_chart(
          "Overall Vitamin A Target (6 - 59 mos)<br><span"
          " style='font-size:0.75em;color:#64748b'>(Acc: Cols Q+R | Target:"
          " targetVitA C2)</span>",
          int(vit_a_acc_total),
  ...
  if not df_target_raw.empty:
    ...
    if t_bakuna_col and col_t_6_59:
      temp_target_df = df_target_raw.copy()

      temp_target_df["Bakuna Center Name"] = (
          temp_target_df[t_bakuna_col]
          .astype(str)
          .str.replace("DISTRICT HEALTH CENTER", "DHC", regex=False)
          .str.replace("HEALTH SERVICES OFFICE - PUBLIC HEALTH LABORATORY", "HSO-PHL", regex=False)
          .str.strip()
      )

      temp_target_df["Target_6_59"] = temp_target_df[col_t_6_59].apply(
          lambda val: clean_numeric_sum(pd.Series([val]))
      )

      dhc_targets = (
          temp_target_df.groupby("Bakuna Center Name")["Target_6_59"]
          .sum()
          .reset_index()
      )
  ...
  with dhc_col1:
    ...
    if not mr_df.empty:
      ...
      dhc_mr_df["Bakuna Center Name"] = (
          dhc_mr_df[col_bakuna]
          .astype(str)
          .str.replace("DISTRICT HEALTH CENTER", "DHC", regex=False)
          .str.replace("HEALTH SERVICES OFFICE - PUBLIC HEALTH LABORATORY", "HSO-PHL", regex=False)
          .str.strip()
      ...
      mr_summary = (
          dhc_mr_df.groupby("Bakuna Center Name")[
              ["MR_6_12", "MR_13_23", "MR_24_59"]
          ]
          .sum()
          .reset_index()
      )
      ...
      if not dhc_targets.empty:
        mr_summary = pd.merge(
            mr_summary, dhc_targets, on="Bakuna Center Name", how="left"
        ).fillna(0)
        mr_summary["Target (6-59mos)"] = mr_summary["Target_6_59"].astype(int)
        mr_summary["% Accomplishment"] = (
            (mr_summary["Total MR"] / mr_summary["Target_6_59"] * 100)
            .round(1)
            .fillna(0)
        )
        mr_summary.drop(columns=["Target_6_59"], inplace=True)
      ...
      if not dhc_targets.empty:
        tot_target = mr_summary["Target (6-59mos)"].sum()
        tot_mr = mr_summary["Total MR"].sum()
        total_row_dict["Target (6-59mos)"] = tot_target
        total_row_dict["% Accomplishment"] = (
            round((tot_mr / tot_target * 100), 1) if tot_target > 0 else 0
        )

      total_row = pd.DataFrame([total_row_dict])
      mr_summary_final = pd.concat([mr_summary, total_row], ignore_index=True)
      st.dataframe(
          mr_summary_final, use_container_width=True, hide_index=True
      )
...
# --- UPDATED OVERLAY BAR CHART (BOTH ON THE SAME ROW) ---
      fig_mr_bar = go.Figure()

      # 1. Background Bar: Target Goal (Wider)
      if "Target (6-59mos)" in mr_summary.columns:
        fig_mr_bar.add_trace(
            go.Bar(
                y=mr_summary["Bakuna Center Name"],
                x=mr_summary["Target (6-59mos)"],
                orientation="h",
                name="Target Goal",
                marker=dict(color="#f7d07e"),  # Soft red target background
                width=0.6,  # Wider width to sit behind actual
                text=[f"Target: {val:,}" for val in mr_summary["Target (6-59mos)"]],
                textposition="inside",
                insidetextanchor="end",
            )
        )

      # 2. Foreground Bar: Actual MR Doses (Thinner, layered on top)
      fig_mr_bar.add_trace(
          go.Bar(
              y=mr_summary["Bakuna Center Name"],
              x=mr_summary["Total MR"],
              orientation="h",
              name="Actual MR Doses",
              marker=dict(color="#1f77b4"),  # Solid blue actual progress
              width=0.35,  # Narrower width so target remains visible
              text=[
                  f"{val:,} ({pct}%)"
                  if "% Accomplishment" in mr_summary.columns
                  else f"{val:,}"
                  for val, pct in zip(
                      mr_summary["Total MR"],
                      mr_summary.get(
                          "% Accomplishment", [0] * len(mr_summary)
                      ),
                  )
              ],
      ...
      fig_mr_bar.update_layout(
          title="<b>Total MR Doses vs Target Goal by Bakuna Center</b>",
          barmode="overlay",  # Places both bars on the exact same row space
          xaxis_title="Doses",
          yaxis_title="",
          yaxis=dict(autorange="reversed"),
          height=max(400, len(mr_summary) * 35),
  ...
  with dhc_col2:
    ...
    if not vit_a_df.empty:
      ...
      dhc_vit_df["Bakuna Center Name"] = (
          dhc_vit_df[col_bakuna]
          .astype(str)
          .str.replace("DISTRICT HEALTH CENTER", "DHC", regex=False)
          .str.replace("HEALTH SERVICES OFFICE - PUBLIC HEALTH LABORATORY", "HSO-PHL", regex=False)
          .str.strip()
      ...
      dhc_vit_df["Vit_12_59"] = (
          pd.to_numeric(
              dhc_vit_df[col_vit_12_59]
              .astype(str)
              .str.replace(",", "")
              .str.strip(),
              errors="coerce",
      ...
      vit_summary = (
          dhc_vit_df.groupby("Bakuna Center Name")[["Vit_6_11", "Vit_12_59"]]
          .sum()
          .reset_index()
      )
      vit_summary["Total Vit A"] = (
          vit_summary["Vit_6_11"] + vit_summary["Vit_12_59"]
      )
      ...
      vit_summary = vit_summary.sort_values(
          by="Total Vit A", ascending=False
      )
      ...
      vit_summary_final = pd.concat(
          [vit_summary, total_vit_row], ignore_index=True
      )
      st.dataframe(
          vit_summary_final, use_container_width=True, hide_index=True
      )
  ...
  for bgy in heatmap_barangays:
    if not filtered_df.empty:
      ...
      mr_doses_val = (
          ...
          .sum()
          .sum()
          if not bgy_data.empty
          else 0
      )

      vit_a_val = (
          bgy_data[heatmap_vit_cols]
          .apply(
              lambda col: pd.to_numeric(
                  col.astype(str).str.replace(",", "").str.strip(),
                  errors="coerce",
  ...
  if not heatmap_df.empty and heatmap_df["Total Accomplishment"].sum() > 0:
    heatmap_df = heatmap_df.sort_values(
        by="Total Accomplishment", ascending=False
    )
    st.download_button(
    ...
    heatmap_melted = (
        heatmap_df.sort_values(by="Total Accomplishment", ascending=True).melt(
            id_vars=["Barangay"],
    ...
    fig_table_heatmap = px.density_heatmap(
        ...
        text_auto=True,
        title=(
            "<b>Barangay Accomplishment Heatmap (MR: Cols K-M | Vit A: Cols"
            " Q-R)</b>"
        ),
    )
  ...
  with b_right_col:
    ...
    if vit_a_target_cols and not vit_a_df.empty:
      ...
      vit_a_bgy_summary = (
          vit_a_bgy_df.groupby(col_barangay)["Total_Vit_A"].sum().reset_index()
      )
      vit_a_bgy_summary = vit_a_bgy_summary[
          vit_a_bgy_summary["Total_Vit_A"] > 0
      ]
  ...
  with b_left_col:
    ...
    if mr_dose_cols and not mr_df.empty:
      ...
      mr_bgy_summary = (
          mr_bgy_df.groupby(col_barangay)["Total_MR_Doses"].sum().reset_index()
      )
    ...
    if mr_zero_cols and not mr_df.empty:
      ...
      mr_zero_bgy_summary = (
          mr_zero_bgy_df.groupby(col_barangay)["Total_MR_Zero"]
          .sum()
          .reset_index()
      )
      mr_zero_bgy_summary = mr_zero_bgy_summary[
          mr_zero_bgy_summary["Total_MR_Zero"] > 0
      ]
  ...
  total_deferrals_val = (
      clean_numeric_sum(filtered_df[col_deferrals])
      if (not filtered_df.empty and col_deferrals)
      else 0
  )
  total_refusals_val = (
      clean_numeric_sum(filtered_df[col_refusals])
      if (not filtered_df.empty and col_refusals)
      else 0
  )
  ...
  with def_col2:
    ...
    fig_def_ref.update_layout(
        xaxis_title="", yaxis_title="Count", showlegend=False, height=300
    )
  ...
  with tbl_col1:
    ...
    def_reason_cols = (
        df_raw.columns[27:41].tolist() if len(df_raw.columns) >= 41 else []
    )
    def_reasons_data = []
    ...
    for col in def_reason_cols:
      ...
      clean_name = (
          str(col)
          .replace("Deferral Reason -", "")
          .replace("Deferral Reason:", "")
          .strip()
      )
      def_reasons_data.append(
          {"Deferral Reason": clean_name, "Count": int(cnt)}
      )

    if def_reasons_data:
      def_reasons_df = pd.DataFrame(def_reasons_data).sort_values(
  ...
  if not df_target_raw.empty and len(df_target_raw.columns) > 1:
    ...
    all_target_bgys = sorted(
        [b for b in all_target_bgys if b and b.upper() != "NAN"]
    )
    ...
    if all_target_bgys:
      mr_submitted_bgys = set(
          df_raw[
              df_raw[col_response]
              .astype(str)
              .str.contains("Measles|MR", case=False, na=False)
          ][col_barangay]
          .dropna()
          .astype(str)
          .str.strip()
      ...
      vit_a_submitted_bgys = set(
          df_raw[
              df_raw[col_response]
              .astype(str)
              .str.contains("Vitamin A", case=False, na=False)
          ][col_barangay]
          .dropna()
          .astype(str)
          .str.strip()
      ...
      status_filter = st.radio(
          "Filter View:",
          [
              "Show All Target Barangays",
              "Show Pending Only",
              "Show Fully Submitted Only",
          ],
          horizontal=True,
      )
      ...
      for bgy in all_target_bgys:
        ...
        has_mr = (
            "Submitted"
            if bgy_upper in mr_submitted_bgys
            else "Not Yet Submitted"
        )
        has_vit_a = (
            "Submitted"
            if bgy_upper in vit_a_submitted_bgys
            else "Not Yet Submitted"
        )
      ...
      pending_mr_cnt = sum(
          ...
          if r["Measles-Rubella (MR) Status"] == "Not Yet Submitted"
      )
      pending_vit_cnt = sum(
          1
          for r in submission_records
          if r["Vitamin A Status"] == "Not Yet Submitted"
      )
      fully_sub_cnt = sum(
          1
          for r in submission_records
          if r["Overall Status"] == "Fully Submitted"
      )
    ...
    st.warning(
        "Target sheet is empty or does not contain Column B (Barangay Name)."
    )
