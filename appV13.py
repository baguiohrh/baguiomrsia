...
# --- SUB-SECTION 2: VITAMIN A ---
    ...
    st.info("ℹ️ **Note:** Target is for the entire city of Baguio; no disaggregation per barangay.")
    ...
    # 1. Measles-Rubella Table & Chart (Actual vs Target)
    with dhc_col1:
        ...
        if not mr_df.empty:
            ...
            # --- EXTRACT TARGET DATA BY BAKUNA CENTER ---
            # Bakuna Center Name [Col C / idx 2], 6-59 mos Total [Col F / idx 5]
            if not df_target_raw.empty and len(df_target_raw.columns) > 5:
                t_bakuna_col_dhc = df_target_raw.columns[2]
                t_target_col_dhc = df_target_raw.columns[5]

                df_target_temp = df_target_raw[[t_bakuna_col_dhc, t_target_col_dhc]].copy()
                df_target_temp[t_target_col_dhc] = pd.to_numeric(
                    df_target_temp[t_target_col_dhc].astype(str).str.replace(",", "").str.strip(), 
                    errors='coerce'
                ).fillna(0)

                target_by_bc = df_target_temp.groupby(t_bakuna_col_dhc)[t_target_col_dhc].sum().reset_index()
                target_by_bc.columns = ['Bakuna Center Name', 'Target MR']

                # Merge actual accomplishments with target values
                mr_chart_df = pd.merge(mr_summary, target_by_bc, on='Bakuna Center Name', how='outer').fillna(0)
            else:
                mr_chart_df = mr_summary.copy()
                mr_chart_df['Target MR'] = 0

            # Melt for grouped horizontal bar chart
            mr_chart_melted = mr_chart_df.melt(
                id_vars=['Bakuna Center Name'],
                value_vars=['Total MR', 'Target MR'],
                var_name='Type',
                value_name='Count'
            )
            mr_chart_melted['Type'] = mr_chart_melted['Type'].map({
                'Total MR': 'Actual Doses',
                'Target MR': 'Target'
            })

            # Sort centers based on actual accomplishment
            order_bc = mr_chart_df.sort_values(by='Total MR', ascending=True)['Bakuna Center Name'].tolist()
            ...
            fig_mr_bar = px.bar(
                ...
                orientation='h',
                text='Count',
                title='<b>Measles-Rubella Doses: Actual vs Target by Bakuna Center</b>',
                category_orders={'Bakuna Center Name': order_bc},
                color_discrete_map={
                    'Actual Doses': '#1f77b4',
                    'Target': '#ff7f0e'
                }
            ...
            fig_mr_bar.update_layout(
                ...
                yaxis_title="",
                height=max(380, len(mr_chart_df) * 45),
